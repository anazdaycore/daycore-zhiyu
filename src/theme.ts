...
// 自定义主题没有静态 CSS 段落，它靠 base（dark?'night':(内置 base||'sunset')）
// 继承深浅文字的整套处理，再把自己的 variables 逐条写到 :root 覆盖底座。
// 缺省纸色是 sunset（牛皮纸），不是 sky —— 原型 index.html 的 flApplyTheme 兜底就是 sunset。
//
...
export function applyTheme(id: string | undefined, custom: CustomTheme[]): void {
  ...
  if (found) {
    el.setAttribute('data-theme', found.dark ? 'night' : isBuiltin(found.base || '') ? found.base! : DEFAULT);
    for (const [k, v] of Object.entries(found.variables)) {
      if (v) el.style.setProperty(k, v);
    }
  } else {
    el.setAttribute('data-theme', isBuiltin(chosen) ? chosen : DEFAULT);
  }
}
...
export function initialThemeAttr(id: string | undefined): string {
  const chosen = id || DEFAULT;
  return isBuiltin(chosen) ? chosen : DEFAULT;
}
